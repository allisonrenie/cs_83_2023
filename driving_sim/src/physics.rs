/// This file defines most of the driving simulation specific physics, 
/// including the workings of the engine. It also handles controller
/// and keyboard inputs to controll the car. 
/// 
/// Below are the controller and keyboard button mappings:
/// 
...
/// Engine struct that holds information about the car's engine
#[derive(Component)]
pub struct Engine 
{
    rpms: f32,
...
/// Handles controller and keyboard input for "gas pedal" and gear shifting
...
/// Handles steering for both keyboard and controller
#[derive(Component)]
/// Steering struct to flag steered joints
pub struct Steering;
pub fn steering_system(keys: Res<Input<KeyCode>>, mut joints: Query<(&mut Joint, &Steering)>, time: Res<Time>, gamepads: Res<Gamepads>, button_axes: Res<Axis<GamepadButton>>,axes: Res<Axis<GamepadAxis>>) {
    ...
    // Adding steering angle to joints
...
/// Breaking struct
pub struct Breaking;
/// Implements breaking for keyboard and controller
pub fn breaking_system(mut joints: Query<(&mut Joint, &DrivenWheel)>, key: Res<Input<KeyCode>>,keys: Res<Input<KeyCode>>,gamepads: Res<Gamepads>, button_inputs: Res<Input<GamepadButton>>){
    ...
    /// Handling hard and soft breaking by altering joint values
    for (mut joint, _) in joints.iter_mut() {
        /// Hard
        if breaking == 100.0{
            joint.qd = 0.0;
...
#[derive(Component)]
/// DrivenWheels to flag driven wheels
pub struct DrivenWheel;
/// Functions like an engine. Engine RPMs are matched to torque and gear ratio is found from 
/// current gear. Torque, gear ratio, and a "differential" combine to create a speed target
/// for the car. If the car is in neutral, this will be disregarded. If the car is in reverse, 
/// speed is reversed. Joint values are updated to reflect new speed target.
pub fn driven_wheel_system(mut joints: Query<(&mut Joint, &DrivenWheel, &mut Engine)>) {
    ...
    {  
        ...
        /// Constant that acts like a differential 
        /// CHANGE THIS to make the car more rective to speed, higher=less reactive/less fast
        let mut differential = 4.0; 
